...
function mapPost(row:Row,viewer:"customer"|"provider",viewerLat:number|null,viewerLng:number|null){const lat=row.latitude===null?null:Number(row.latitude),lng=row.longitude===null?null:Number(row.longitude);const canDistance=viewerLat!==null&&viewerLng!==null&&lat!==null&&lng!==null&&Number.isFinite(lat)&&Number.isFinite(lng);return {id:String(row.id),status:String(row.status),deviceCategory:String(row.device_category),brand:String(row.brand??""),modelKey:String(row.model_key),modelLabel:String(row.model_label),issueKey:String(row.issue_key),problemDetail:String(row.problem_detail),customerName:viewer==="customer"?String(row.customer_display_name):safeCustomerName(String(row.customer_display_name)),city:String(row.city),region:String(row.region??""),countryCode:String(row.country_code),distanceKm:canDistance?Math.round(distanceKm(viewerLat!,viewerLng!,lat!,lng!)*10)/10:null,photoCount:Number(row.photo_count)||0,offerCount:Number(row.offer_count)||0,acceptedOfferId:row.accepted_offer_id?String(row.accepted_offer_id):null,primaryPhotoId:row.primary_photo_id?String(row.primary_photo_id):null,expiresAt:String(row.expires_at),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
